...
class BVNVerify extends Controller
{
    ...
    public function send_bvn_otp(Request $request){
        ...
        $member->bvn = $validated["bvn"];
         if($member->bvn_verification_attempts == null){
            $member->bvn_verification_attempts =  1;
         }else{
            $member->bvn_verification_attempts = intval($member->bvn_verification_attempts) + 1;
         }
        $member->save();
    ...
    public function verify_bvn_otp(Request $request){
        ...
        $member->phone = $decodedBody["data"]["providerResponse"]["phoneNumber1"];
